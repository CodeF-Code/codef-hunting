...
RegisterNetEvent("black_hunting:harvestAnimal", function(data)
    local playerPed = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(playerPed)

    local dist = #(playerCoords - data.coords)

    if dist > 8.0 then return print("black HUNTING >> PLAYER MIGHT BE CHEATING ID: " .. source) end
...
RegisterNetEvent("black_hunting:cookItem", function(data)
...
RegisterNetEvent("black_hunting:takeCampfire", function(data)
    local sourcePed = GetPlayerPed(source)

    if #(GetEntityCoords(sourcePed) - data.coords) > 4.0 then return print("black HUNTING >> PLAYER MIGHT BE CHEATING ID: " .. source) end
    framework.addItems({ target = source, items = Config.Campfire.campfireItem })
end)


RegisterNetEvent("black_hunting:sellBuyItem", function(data)
    local source = source
    if data.buy then
        if framework.hasMoney(source) >= data.price then
            ...
            TriggerClientEvent("black_hunting:showNotification", source, locale("not_enough_money"))
        end
    else
        if framework.hasItems({ target = source, items = { { item = data.item, quantity = data.quantity } } }) then
            ...
            TriggerClientEvent("black_hunting:showNotification", source, locale("not_enough_item"))
        end
    end
end)

RegisterNetEvent("black_hunting:finishMission", function(data)
    local source = source
    local playerPed = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(playerPed)
    local dist = #(playerCoords - Config.HuntMaster.coords.xyz)
    if dist > 3.0 then return print("black HUNTING >> PLAYER MIGHT BE CHEATING ID: " .. source) end
...
RegisterNetEvent("black_hunting:missionTime", function(data)
...
lib.callback.register('black_hunting:canDoMission', function(source, id, delay)
    local playerIdentifier = GetPlayerIdentifierByType(source, "license")
...
lib.callback.register('black_hunting:hasItems', function(source, items)
    return framework.hasItems({ target = source, items = items })
end)

lib.versionCheck('Arius-Development/black_hunting')
