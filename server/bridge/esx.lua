...
function framework.addItems(data)
    ...
    if type(data.items) == "table" then
        for _, item in pairs(data.items) do
            xPlayer.fusion:giveItemSecure(item.item, (item?.quantity or 1))
        end
    else
        xPlayer.fusion:giveItemSecure(data.items, 1)
    end
end
...
if Config.Campfire.enable then
    ESX.RegisterUsableItem(Config.Campfire.campfireItem, function(source, name, item)
        ...
        TriggerClientEvent("black_hunting:useCampfire", source)
    end)
end
