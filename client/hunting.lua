    ...
    if value then
        ...
        if currentZone then
            if currentZone.allowedWeapons then
                if not utils.validWeapon(currentZone.allowedWeapons, value) then
                    lib.alertDialog({
                        header = locale("weapon not allowed title"),
                        content = locale("Unauthorized weapon not allowed in Hunting Zone"),
                        centered = true,
                        cancel = false
                    })
                    disableWeapon()
        ...
        if not currentZone then
            for _, zone in pairs(Config.HuntingZones) do
                if zone.allowedWeapons and utils.validWeapon(zone.allowedWeapons, value) then
                    lib.alertDialog({
                        header = locale("weapon not allowed title"),
                        content = locale("weapon can only use in hunting zone"),
                        centered = true,
                        cancel = false
                    })
                    disableWeapon()
                ...
                CreateThread(function()
                    ...
                    if not Config.Target then
                        while DoesEntityExist(entity) do
                            ...
                            if dist <= 3.0 then
                                sleep = 0
                                utils.drawText3D(entityCoords, locale("haverest_animal"), 1, 0)
...
function harvestAnimal(animal, entityCoords, entity)
    ...
    if canHarvest then
        ...
        TriggerServerEvent("black_hunting:harvestAnimal", data)
    else
...
RegisterNetEvent("black_hunting:trackAnimal", trackAnimal)
...
RegisterNetEvent("black_hunting:placeBait", placeBait)
